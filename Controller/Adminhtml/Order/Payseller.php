...
namespace Hts\Marketplace\Controller\Adminhtml\Order;
...
use Hts\Marketplace\Helper\Email as MpEmailHelper;
use Hts\Marketplace\Model\SellertransactionFactory;
use Hts\Marketplace\Model\SaleperpartnerFactory;
use Hts\Marketplace\Model\OrdersFactory;
...
class Payseller extends \Magento\Backend\App\Action
{
     ...
     * @param \Hts\Marketplace\Model\SaleslistFactory  $salesListModel
     */
    public function __construct(
        ...
        \Hts\Marketplace\Model\SaleslistFactory $salesListModel
    ) {
        $this->filter = $filter;
        ...
        try {
            ...
            foreach ($collection as $row) {
                ...
                $orderinfo = $orderinfo."<tr>
                    ...
                    <td class='item-price'>".$order->formatBasePrice($totalamount - $actparterprocost)."</td>
                    <td class='item-price'>".$order->formatBasePrice($actparterprocost).'</td>
                </tr>';
            }
            if ($actparterprocost) {
                ...
                if ($uniqueId != '') {
                    ...
                    $this->notificationHelper->saveNotification(
                        \Hts\Marketplace\Model\Notification::TYPE_TRANSACTION,
                        $transid,
                        $transid
                    );
                }
    ...
    {
        return $this->_authorization->isAllowed('Hts_Marketplace::seller');
    }
}
