...
class Navigation extends \Magento\Framework\View\Element\Html\Link
{
        ...
        if ($productStatus == 1) {
            return __(
                sprintf(
                    ...
                    '<span class="ht-focus">'.$product->getName().'</span>'
                )
            );
        } else {
            return __(
    ...
    public function getOrderNotificationDesc($orderId)
    {
        ...
        return __(
            sprintf(
                ...
                '<span class="ht-focus">'.$productNames.'</span>',
                '<span class="ht-focus">#'.$order->getIncrementId().'</span>'
            )
        );
    }
    ...
    public function getTransactionNotifyTitle($transactionId)
    {
        $transactionBlock = $this->getLayout()->createBlock(
            \Hts\Marketplace\Block\Transaction\View::class
        );
    ...
    public function getTransactionNotifyDesc($id)
    {
        ...
        $desc = __(sprintf(
            ...
            '<span class="ht-focus">#'.$orderId.'</span>',
            '<span class="ht-focus">'.$sellerTransation->getMethod().'</span>'
        ));
        return $desc;
    }
    /**
