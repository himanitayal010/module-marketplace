        ...
        _create: function () {
            ...
            $("#frontend_input").click(function () {
                if (attr_options !== 0 && select !== 1) {
                    attr_options=$(".ht-mp-option-box").clone();
                }
            });
            
            $("#associate-product").delegate('.ht-mp-headcus input','focusout',function () {
                    $(this).attr('value',$(this).val());
            });
            
            $("#associate-product").delegate('.ht-mp-headcus input[type="checkbox"]','focusout',function () {
                if ($(this).is(":checked")) {
...
$(this).removeAttr("checked");
            ...
            $("#frontend_input").change(function () {
                $('.val_required').show();
                $(".ht-mp-option-box").remove();
                if ($("#frontend_input").val() == "multiselect" || $("#frontend_input").val() == "select") {
                    if (attr_options===0) {
                        var headone=$('<div/>').addClass("ht-mp-option-box")
                        .append(
                            $('<ul/>').addClass("ht-mp-headcus ul_first")
                        ...
                        $('#cust').append($('<div/>').addClass("ht-mp-option-box").append(attr_options.html()));
                    }
                } else {
                    select=1;
                }
            ...
            $("#associate-product").delegate(".deletecusopt","click",function () {
                $(this).parents(".ht-mp-headcus").remove();
            });

            $("#associate-product").delegate(".attroptions","click",function () {
                var addcust = $('<ul/>').addClass('ht-mp-headcus')
                                .append($('<li/>')
                ...
                $('.ht-mp-option-box').append(addcust);
                fcop++;
            });
