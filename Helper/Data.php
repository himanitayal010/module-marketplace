...
use Hts\Marketplace\Model\ResourceModel\Product\CollectionFactory as MpProductCollection;
use Hts\Marketplace\Model\ResourceModel\Feedback\CollectionFactory as MpFeedbackCollection;
use Hts\Marketplace\Model\ResourceModel\Seller\CollectionFactory as SellerCollection;
use Magento\Catalog\Model\ResourceModel\Product\CollectionFactory as ProductCollection;
use Magento\Framework\DB\Sql\Expression as SqlExpression;
use Magento\Framework\DB\Select as DBSelect;
use Magento\Cms\Model\Wysiwyg\Config;
use Magento\Catalog\Model\Product\Visibility;
use Hts\Marketplace\Model\ControllersRepository;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @param \Hts\Marketplace\Model\Seller $marketplaceSeller
    ...
    public function __construct(
        ...
        \Hts\Marketplace\Model\Seller $marketplaceSeller,
     ...
     *
     * @return \Hts\Marketplace\Model\ResourceModel\Seller\Collection
     */
     ...
     *
     * @return \Hts\Marketplace\Model\ResourceModel\Product\Collection
     */
    ...
    public function getAllowedProductType()
    {
        ...
        foreach ($data as $key => $value) {
            if ($value == 'grouped') {
                if ($this->_moduleManager->isEnabled('Hts_MpGroupedProduct')) {
            ...
            if ($value == 'bundle') {
                if ($this->_moduleManager->isEnabled('Hts_MpBundleProduct')) {
    ...
    {
        if ($this->_moduleManager->isEnabled('Hts_MpSellerGroup')) {
            return true;
        }
        return false;
    }
    ...
    public function isAllowedAction($actionName = '')
    {
        if ($this->isSellerGroupModuleInstalled()) {
            $sellerGroupHelper = $this->_objectManager->create(
                \Hts\MpSellerGroup\Helper\Data::class
            );
            ...
            $sellerGroupTypeRepository = $this->_objectManager->create(
                \Hts\MpSellerGroup\Api\SellerGroupTypeRepositoryInterface::class
            );
    ...
    {
        if ($this->_moduleManager->isEnabled('Hts_MpSellerCoupons')) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->_moduleManager->isEnabled('Hts_Mpcashondelivery')) {
            return true;
        }
    ...
    {
        if ($this->_moduleManager->isEnabled('Hts_MpSellerProductSearch')) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->_moduleManager->isEnabled('Hts_Mpsellerslider')) {
            return true;
        }
    ...
    protected function getSellerId($mpassignproductId, $proid)
    {
        ...
        if ($mpassignproductId) {
            $this->assignItemsFactory = ObjectManager::getInstance()->create(
                \Hts\MpAssignProduct\Model\ItemsFactory::class
            );
